...
use ApiClients\Client\GitHubAE\Operation\Repos\AddStatusCheckContexts;
...
final class AddStatusCheckContextsTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], '[' . Schema\Operation\Repos\AddStatusCheckContexts\Response\Applicationjson\H200::SCHEMA_EXAMPLE_DATA . ']');
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/repos/generated_null/generated_null/branches/generated_null/protection/required_status_checks/contexts', Argument::type('array'), Schema\Repos\AddStatusCheckContexts\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(AddStatusCheckContexts::OPERATION_MATCH, (static function (array $data): array {
