...
final class ListForSuiteTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    ...
    public function httpCode_200_responseContentType_application_json()
    {
        ...
        $browser->request('GET', '/repos/generated_null/generated_null/check-suites/13/check-runs?check_name=generated_null&status=generated_null&filter=generated_null&per_page=13&page=13', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Checks\ListForSuite::OPERATION_MATCH, (static function (array $data) : array {
