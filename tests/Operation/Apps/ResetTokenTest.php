...
final class ResetTokenTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    public function httpCode_200_requestContentType_application_json_responseContentType_application_json()
    {
        ...
        $browser->request('PATCH', '/applications/generated_null/token', \Prophecy\Argument::type('array'), Schema\Apps\ResetToken\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Apps\ResetToken::OPERATION_MATCH, (static function (array $data) : array {
