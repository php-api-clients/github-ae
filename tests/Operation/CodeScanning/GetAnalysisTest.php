...
final class GetAnalysisTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    public function httpCode_200_responseContentType_application_json()
    {
        ...
        $browser->request('GET', '/repos/generated_null/generated_null/code-scanning/analyses/13', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\CodeScanning\GetAnalysis::OPERATION_MATCH, (static function (array $data) : array {
     ...
     * @test
     */
    public function httpCode_200_responseContentType_application_json_sarif()
    {
        $response = new \React\Http\Message\Response(200, array('Content-Type' => 'application/json+sarif'), Schema\Operation\CodeScanning\GetAnalysis\Response\ApplicationjsonPlusSarif\H200::SCHEMA_EXAMPLE_DATA);
        $auth = $this->prophesize(\ApiClients\Contracts\HTTP\Headers\AuthenticationInterface::class);
        $auth->authHeader(\Prophecy\Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(\React\Http\Browser::class);
        $browser->withBase(\Prophecy\Argument::any())->willReturn($browser->reveal());
