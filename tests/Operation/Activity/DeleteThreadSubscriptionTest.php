...
final class DeleteThreadSubscriptionTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    public function httpCode_403_responseContentType_application_json()
    {
        ...
        $browser->request('DELETE', '/notifications/threads/13/subscription', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Activity\DeleteThreadSubscription::OPERATION_MATCH, (static function (array $data) : array {
            $data['thread_id'] = 13;
            return $data;
        })(array()));
    }
