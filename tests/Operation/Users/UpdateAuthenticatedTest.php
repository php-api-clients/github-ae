...
final class UpdateAuthenticatedTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], Schema\PrivateUser::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('PATCH', '/user', Argument::type('array'), Schema\Users\UpdateAuthenticated\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
    ...
    public function operations_httpCode_200_requestContentType_application_json_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('PATCH', '/user', Argument::type('array'), Schema\Users\UpdateAuthenticated\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = await($client->operations()->users()->updateAuthenticated(json_decode(Schema\Users\UpdateAuthenticated\Request\ApplicationJson::SCHEMA_EXAMPLE_DATA, true)));
    }
