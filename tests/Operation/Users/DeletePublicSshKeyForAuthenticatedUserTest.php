...
final class DeletePublicSshKeyForAuthenticatedUserTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    public function httpCode_404_responseContentType_application_json()
    {
        ...
        $browser->request('DELETE', '/user/keys/13', \Prophecy\Argument::type('array'), \Prophecy\Argument::any())->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Users\DeletePublicSshKeyForAuthenticatedUser::OPERATION_MATCH, (static function (array $data) : array {
            $data['key_id'] = 13;
            return $data;
        })(array()));
    }
