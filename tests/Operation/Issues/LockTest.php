...
final class LockTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    public function httpCode_403_requestContentType_application_json_responseContentType_application_json()
    {
        ...
        $browser->request('PUT', '/repos/generated_null/generated_null/issues/13/lock', \Prophecy\Argument::type('array'), Schema\Issues\Lock\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Issues\Lock::OPERATION_MATCH, (static function (array $data) : array {
