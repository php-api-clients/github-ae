...
use ApiClients\Client\GitHubAE\Operation\Projects\Update;
...
final class UpdateTest extends AsyncTestCase
{
    ...
    public function httpCode_200_requestContentType_application_json_responseContentType_application_json(): void
    {
        ...
        $browser->request('PATCH', '/projects/13', Argument::type('array'), Schema\Projects\Update\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
    ...
    public function httpCode_403_requestContentType_application_json_responseContentType_application_json(): void
    {
        ...
        $response = new Response(403, ['Content-Type' => 'application/json'], Schema\Operation\Projects\Update\Response\Applicationjson\H403::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
