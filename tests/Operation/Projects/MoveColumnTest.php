...
use ApiClients\Client\GitHubAE\Operation\Projects\MoveColumn;
...
final class MoveColumnTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(201, ['Content-Type' => 'application/json'], Schema\Operation\Projects\MoveColumn\Response\Applicationjson\H201::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/projects/columns/13/moves', Argument::type('array'), Schema\Projects\MoveColumn\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(MoveColumn::OPERATION_MATCH, (static function (array $data): array {
            $data['column_id'] = 13;
