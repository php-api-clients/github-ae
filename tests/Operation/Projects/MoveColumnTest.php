...
final class MoveColumnTest extends \WyriHaximus\AsyncTestUtilities\AsyncTestCase
{
    /**
     * @test
     */
    ...
    {
        $response = new \React\Http\Message\Response(201, array('Content-Type' => 'application/json'), Schema\Operation\Projects\MoveColumn\Response\Applicationjson\H201::SCHEMA_EXAMPLE_DATA);
        $auth = $this->prophesize(\ApiClients\Contracts\HTTP\Headers\AuthenticationInterface::class);
        $auth->authHeader(\Prophecy\Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(\React\Http\Browser::class);
        $browser->withBase(\Prophecy\Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(\Prophecy\Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/projects/columns/13/moves', \Prophecy\Argument::type('array'), Schema\Projects\MoveColumn\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(\React\Promise\resolve($response))->shouldBeCalled();
        $client = new \ApiClients\Client\GitHubAE\Client($auth->reveal(), $browser->reveal());
        $client->call(\ApiClients\Client\GitHubAE\Operation\Projects\MoveColumn::OPERATION_MATCH, (static function (array $data) : array {
            $data['column_id'] = 13;
            return $data;
