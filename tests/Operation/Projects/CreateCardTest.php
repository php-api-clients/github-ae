...
use ApiClients\Client\GitHubAE\Operation\Projects\CreateCard;
...
final class CreateCardTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(201, ['Content-Type' => 'application/json'], Schema\ProjectCard::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/projects/columns/13/cards', Argument::type('array'), Schema\Projects\CreateCard\Request\Applicationjson::SCHEMA_EXAMPLE_DATA)->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(CreateCard::OPERATION_MATCH, (static function (array $data): array {
            $data['column_id'] = 13;
    ...
    public function httpCode_422_requestContentType_application_json_responseContentType_application_json(): void
    {
        ...
        $response = new Response(422, ['Content-Type' => 'application/json'], Schema\Operation\Projects\CreateCard\Response\Applicationjson\H422::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
    ...
    public function httpCode_503_requestContentType_application_json_responseContentType_application_json(): void
    {
        ...
        $response = new Response(503, ['Content-Type' => 'application/json'], Schema\Operation\Projects\CreateCard\Response\Applicationjson\H503::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
