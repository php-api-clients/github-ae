...
use ApiClients\Client\GitHubAE\Operation\Search\Code;
...
final class CodeTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(200, ['Content-Type' => 'application/json'], Schema\Operation\Search\Code\Response\Applicationjson\H200::SCHEMA_EXAMPLE_DATA);
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('GET', '/search/code?q=generated_null&sort=generated_null&order=generated_null&per_page=13&page=13', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $client->call(Code::OPERATION_MATCH, (static function (array $data): array {
