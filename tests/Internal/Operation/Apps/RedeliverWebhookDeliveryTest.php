...
/** @covers \ApiClients\Client\GitHubAE\Internal\Operation\Apps\RedeliverWebhookDelivery */
final class RedeliverWebhookDeliveryTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(202, ['Content-Type' => 'application/json'], json_encode(json_decode(Schema\Operations\Apps\RedeliverWebhookDelivery\Response\ApplicationJson\Accepted\Application\Json::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/app/hook/deliveries/11/attempts', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(Internal\Operation\Apps\RedeliverWebhookDelivery::OPERATION_MATCH, (static function (array $data): array {
            $data['delivery_id'] = 11;
    ...
    public function operations_httpCode_202_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('POST', '/app/hook/deliveries/11/attempts', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->apps()->redeliverWebhookDelivery(11);
    }
