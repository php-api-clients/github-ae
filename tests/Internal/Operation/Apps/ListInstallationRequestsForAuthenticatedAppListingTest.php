...
/** @covers \ApiClients\Client\GitHubAE\Internal\Operation\Apps\ListInstallationRequestsForAuthenticatedAppListing */
final class ListInstallationRequestsForAuthenticatedAppListingTest extends AsyncTestCase
{
    /** @test */
    public function call_httpCode_401_responseContentType_application_json_zero(): void
    {
        ...
        $result = $client->call(Internal\Operation\Apps\ListInstallationRequestsForAuthenticatedAppListing::OPERATION_MATCH, (static function (array $data): array {
            $data['per_page'] = 8;
            $data['page']     = 1;
    ...
    public function operations_httpCode_401_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('GET', '/app/installation-requests?per_page=8&page=1', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->apps()->listInstallationRequestsForAuthenticatedAppListing(8, 1);
        foreach ($result as $item) {
        }
    }
