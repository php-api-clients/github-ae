...
/** @covers \ApiClients\Client\GitHubAE\Internal\Operation\Orgs\RedeliverWebhookDelivery */
final class RedeliverWebhookDeliveryTest extends AsyncTestCase
{
    ...
    {
        $response = new Response(202, ['Content-Type' => 'application/json'], json_encode(json_decode(Schema\Operations\Orgs\RedeliverWebhookDelivery\Response\ApplicationJson\Accepted\Application\Json::SCHEMA_EXAMPLE_DATA, true)));
        $auth     = $this->prophesize(AuthenticationInterface::class);
        $auth->authHeader(Argument::any())->willReturn('Bearer beer')->shouldBeCalled();
        $browser = $this->prophesize(Browser::class);
        $browser->withBase(Argument::any())->willReturn($browser->reveal());
        $browser->withFollowRedirects(Argument::any())->willReturn($browser->reveal());
        $browser->request('POST', '/orgs/generated/hooks/7/deliveries/11/attempts', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->call(Internal\Operation\Orgs\RedeliverWebhookDelivery::OPERATION_MATCH, (static function (array $data): array {
            $data['org']         = 'generated';
            $data['hook_id']     = 7;
            $data['delivery_id'] = 11;
    ...
    public function operations_httpCode_202_responseContentType_application_json_zero(): void
    {
        ...
        $browser->request('POST', '/orgs/generated/hooks/7/deliveries/11/attempts', Argument::type('array'), Argument::any())->willReturn(resolve($response))->shouldBeCalled();
        $client = new Client($auth->reveal(), $browser->reveal());
        $result = $client->operations()->orgs()->redeliverWebhookDelivery('generated', 7, 11);
    }
