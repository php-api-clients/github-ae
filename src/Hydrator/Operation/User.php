...
class User implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PrivateUser(array $payload): \ApiClients\Client\GitHubAE\Schema\PrivateUser
    {
        $properties = []; 
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(\ApiClients\Client\GitHubAE\Schema\PrivateUser::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new \ApiClients\Client\GitHubAE\Schema\PrivateUser(...$properties);
        } catch (\Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\PrivateUser', $exception, stack: $this->hydrationStack);
        }
    }
  ...
  0 => 'ApiClients\\Client\\GitHubAE\\Schema\\ValidationError\\Errors',
    ...
    {
        \assert($object instanceof \ApiClients\Client\GitHubAE\Schema\PrivateUser);
        $result = [];
