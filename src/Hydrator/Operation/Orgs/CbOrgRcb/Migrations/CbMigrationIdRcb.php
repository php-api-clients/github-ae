...
class CbMigrationIdRcb implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️Migration(array $payload): \ApiClients\Client\GitHubAE\Schema\Migration
    {
        ...
        try {
            ...
            if ($value === null) {
                ...
                goto after_lockRepositories;
            }

            $properties['lockRepositories'] = $value;

            after_lockRepositories:

            $value = $payload['exclude_metadata'] ?? null;

            if ($value === null) {
                $missingFields[] = 'exclude_metadata';
                goto after_excludeMetadata;
            }

            $properties['excludeMetadata'] = $value;

            after_excludeMetadata:

            $value = $payload['exclude_git_data'] ?? null;

            if ($value === null) {
                $missingFields[] = 'exclude_git_data';
                goto after_excludeGitData;
            }

            $properties['excludeGitData'] = $value;

            after_excludeGitData:

            $value = $payload['exclude_attachments'] ?? null;

            if ($value === null) {
                $missingFields[] = 'exclude_attachments';
                goto after_excludeAttachments;
            }

            $properties['excludeAttachments'] = $value;

            after_excludeAttachments:

            $value = $payload['exclude_releases'] ?? null;

            if ($value === null) {
                $missingFields[] = 'exclude_releases';
                goto after_excludeReleases;
            }

            $properties['excludeReleases'] = $value;

            after_excludeReleases:

            $value = $payload['exclude_owner_projects'] ?? null;

            if ($value === null) {
                $missingFields[] = 'exclude_owner_projects';
                goto after_excludeOwnerProjects;
            }

            $properties['excludeOwnerProjects'] = $value;

            after_excludeOwnerProjects:

            $value = $payload['org_metadata_only'] ?? null;

            if ($value === null) {
                $missingFields[] = 'org_metadata_only';
                goto after_orgMetadataOnly;
            }

            $properties['orgMetadataOnly'] = $value;

            after_orgMetadataOnly:
  ...
  0 => 'ApiClients\\Client\\GitHubAE\\Schema\\Repository',
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️Migration(mixed $object): mixed
    {
        ...
        $lockRepositories = $object->lockRepositories;
        after_lockRepositories:        $result['lock_repositories'] = $lockRepositories;

        
        $excludeMetadata = $object->excludeMetadata;
        after_excludeMetadata:        $result['exclude_metadata'] = $excludeMetadata;

        
        $excludeGitData = $object->excludeGitData;
        after_excludeGitData:        $result['exclude_git_data'] = $excludeGitData;

        
        $excludeAttachments = $object->excludeAttachments;
        after_excludeAttachments:        $result['exclude_attachments'] = $excludeAttachments;

        
        $excludeReleases = $object->excludeReleases;
        after_excludeReleases:        $result['exclude_releases'] = $excludeReleases;

        
        $excludeOwnerProjects = $object->excludeOwnerProjects;
        after_excludeOwnerProjects:        $result['exclude_owner_projects'] = $excludeOwnerProjects;

        
        $orgMetadataOnly = $object->orgMetadataOnly;
        after_orgMetadataOnly:        $result['org_metadata_only'] = $orgMetadataOnly;
