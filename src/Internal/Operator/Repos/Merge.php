...
final readonly class Merge
{
    ...
    /** @return */
    public function call(string $owner, string $repo, array $params): Commit|WithoutBody
    {
        ...
        $result    = await($this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): Commit|WithoutBody {
