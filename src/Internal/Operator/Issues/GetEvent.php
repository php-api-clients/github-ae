...
final readonly class GetEvent
{
    ...
    /** @return */
    public function call(string $owner, string $repo, int $eventId): IssueEvent|array
    {
        $operation = new \ApiClients\Client\GitHubAE\Internal\Operation\Issues\GetEvent($this->responseSchemaValidator, $this->hydrator, $owner, $repo, $eventId);
        $request   = $operation->createRequest();
        $result    = await($this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): IssueEvent|array {
