...
final readonly class ListStargazersForRepo
{
    ...
    public function call(string $owner, string $repo, int $perPage = 30, int $page = 1): SimpleUser|Stargazer
    {
        ...
        $result    = await($this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): SimpleUser|Stargazer {
