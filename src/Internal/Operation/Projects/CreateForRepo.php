...
namespace ApiClients\Client\GitHubAE\Internal\Operation\Projects;
...
final class CreateForRepo
{
    ...
    public function __construct(private readonly SchemaValidator $requestSchemaValidator, private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Projects $hydrator, string $owner, string $repo)
    {
        $this->owner = $owner;
        $this->repo  = $repo;
    }
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\Projects\CreateForRepo\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

        return new Request('POST', str_replace(['{owner}', '{repo}'], [$this->owner, $this->repo], '/repos/{owner}/{repo}/projects'), ['Content-Type' => 'application/json'], json_encode($data));
    }
