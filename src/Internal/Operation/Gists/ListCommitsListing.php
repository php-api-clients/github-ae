...
namespace ApiClients\Client\GitHubAE\Internal\Operation\Gists;
...
final class ListCommitsListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Gists\GistId\Commits $hydrator, string $gistId, int $perPage = 30, int $page = 1)
    {
        $this->gistId  = $gistId;
    ...
    {
        return new Request('GET', str_replace(['{gist_id}', '{per_page}', '{page}'], [$this->gistId, $this->perPage, $this->page], '/gists/{gist_id}/commits' . '?per_page={per_page}&page={page}'));
    }
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                     ...
                     **/
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): Schema\GistCommit {
                            $error = new RuntimeException();
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(Schema\GistCommit::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
