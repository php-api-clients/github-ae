...
final class ListStarredListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Gists\Starred $hydrator, string $since, int $perPage = 30, int $page = 1)
    {
        $this->since   = $since;
    ...
    {
        return new Request('GET', str_replace(['{since}', '{per_page}', '{page}'], [$this->since, $this->perPage, $this->page], '/gists/starred' . '?since={since}&per_page={per_page}&page={page}'));
    }
