...
final class GetAuditLog
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Enterprises\Enterprise\AuditLog $hydrator, string $enterprise, string $phrase, string $after, string $before, string $order, int $page = 1, int $perPage = 30)
    {
    ...
    {
        return new Request('GET', str_replace(['{enterprise}', '{phrase}', '{after}', '{before}', '{order}', '{page}', '{per_page}'], [$this->enterprise, $this->phrase, $this->after, $this->before, $this->order, $this->page, $this->perPage], '/enterprises/{enterprise}/audit-log' . '?phrase={phrase}&after={after}&before={before}&order={order}&page={page}&per_page={per_page}'));
    }
