...
final class ListForTeamDiscussionInOrgListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Orgs\Org\Teams\TeamSlug\Discussions\DiscussionNumber\Reactions $hydrator, string $org, string $teamSlug, int $discussionNumber, string $content, int $perPage = 30, int $page = 1)
    {
        $this->org              = $org;
    ...
    {
        return new Request('GET', str_replace(['{org}', '{team_slug}', '{discussion_number}', '{content}', '{per_page}', '{page}'], [$this->org, $this->teamSlug, $this->discussionNumber, $this->content, $this->perPage, $this->page], '/orgs/{org}/teams/{team_slug}/discussions/{discussion_number}/reactions' . '?content={content}&per_page={per_page}&page={page}'));
    }
