...
final class ListForPullRequestReviewCommentListing
{
    public const OPERATION_ID    = 'reactions/list-for-pull-request-review-comment';
    public const OPERATION_MATCH = 'LIST /repos/{owner}/{repo}/pulls/comments/{comment_id}/reactions';
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Pulls\Comments\CommentId\Reactions $hydrator, string $owner, string $repo, int $commentId, string $content, int $perPage = 30, int $page = 1)
    {
        $this->owner     = $owner;
        $this->repo      = $repo;
    ...
    {
        return new Request('GET', str_replace(['{owner}', '{repo}', '{comment_id}', '{content}', '{per_page}', '{page}'], [$this->owner, $this->repo, $this->commentId, $this->content, $this->perPage, $this->page], '/repos/{owner}/{repo}/pulls/comments/{comment_id}/reactions' . '?content={content}&per_page={per_page}&page={page}'));
    }
