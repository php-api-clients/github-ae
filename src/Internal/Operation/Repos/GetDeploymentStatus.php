...
final class GetDeploymentStatus
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Deployments\DeploymentId\Statuses\StatusId $hydrator, string $owner, string $repo, int $deploymentId, private int $statusId)
    {
        $this->owner        = $owner;
        $this->repo         = $repo;
    ...
    {
        return new Request('GET', str_replace(['{owner}', '{repo}', '{deployment_id}', '{status_id}'], [$this->owner, $this->repo, $this->deploymentId, $this->statusId], '/repos/{owner}/{repo}/deployments/{deployment_id}/statuses/{status_id}'));
    }
