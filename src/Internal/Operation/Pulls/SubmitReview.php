...
final class SubmitReview
{
    ...
    public function __construct(private readonly SchemaValidator $requestSchemaValidator, private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Pulls\PullNumber\Reviews\ReviewId\Events $hydrator, string $owner, string $repo, int $pullNumber, int $reviewId)
    {
        $this->owner      = $owner;
        $this->repo       = $repo;
    ...
    {
        $this->requestSchemaValidator->validate($data, Reader::readFromJson(Schema\Pulls\SubmitReview\Request\ApplicationJson::SCHEMA_JSON, \cebe\openapi\spec\Schema::class));

        return new Request('POST', str_replace(['{owner}', '{repo}', '{pull_number}', '{review_id}'], [$this->owner, $this->repo, $this->pullNumber, $this->reviewId], '/repos/{owner}/{repo}/pulls/{pull_number}/reviews/{review_id}/events'), ['Content-Type' => 'application/json'], json_encode($data));
    }
