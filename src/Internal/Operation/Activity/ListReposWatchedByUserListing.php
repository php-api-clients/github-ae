...
final class ListReposWatchedByUserListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Users\Username\Subscriptions $hydrator, string $username, int $perPage = 30, int $page = 1)
    {
    ...
    {
        return new Request('GET', str_replace(['{username}', '{per_page}', '{page}'], [$this->username, $this->perPage, $this->page], '/users/{username}/subscriptions' . '?per_page={per_page}&page={page}'));
    }
