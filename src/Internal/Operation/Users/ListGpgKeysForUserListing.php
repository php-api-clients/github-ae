...
final class ListGpgKeysForUserListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Users\Username\GpgKeys $hydrator, string $username, int $perPage = 30, int $page = 1)
    {
    ...
    {
        return new Request('GET', str_replace(['{username}', '{per_page}', '{page}'], [$this->username, $this->perPage, $this->page], '/users/{username}/gpg_keys' . '?per_page={per_page}&page={page}'));
    }
                ...
                switch ($code) {
                    ...
                    case 200:
                        return Observable::fromArray($body, new ImmediateScheduler())->map(function (array $body): Schema\GpgKey {
                            ...
                            try {
                                $this->responseSchemaValidator->validate($body, Reader::readFromJson(Schema\GpgKey::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
