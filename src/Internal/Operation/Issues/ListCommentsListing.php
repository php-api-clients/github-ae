...
final class ListCommentsListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Internal\Hydrator\Operation\Repos\Owner\Repo\Issues\IssueNumber\Comments $hydrator, string $owner, string $repo, int $issueNumber, string $since, int $perPage = 30, int $page = 1)
    {
        $this->owner       = $owner;
        $this->repo        = $repo;
    ...
    {
        return new Request('GET', str_replace(['{owner}', '{repo}', '{issue_number}', '{since}', '{per_page}', '{page}'], [$this->owner, $this->repo, $this->issueNumber, $this->since, $this->perPage, $this->page], '/repos/{owner}/{repo}/issues/{issue_number}/comments' . '?since={since}&per_page={per_page}&page={page}'));
    }
