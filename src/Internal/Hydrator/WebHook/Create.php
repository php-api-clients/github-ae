...
use ApiClients\Client\GitHubAE\Schema\WebhookCreate;
...
class Create implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubAE\Schema\WebhookCreate' => $this->hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️WebhookCreate($payload),
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️WebhookCreate(array $payload): WebhookCreate
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            $value = $payload['pusher_type'] ?? null;

            if ($value === null) {
                $missingFields[] = 'pusher_type';
                goto after_pusherType;
            }

            $properties['pusherType'] = $value;
            ...
            $value = $payload['ref_type'] ?? null;

            if ($value === null) {
                $missingFields[] = 'ref_type';
                goto after_refType;
            }

            $properties['refType'] = $value;
            ...
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\WebhookCreate', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(WebhookCreate::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new WebhookCreate(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\WebhookCreate', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubAE\Schema\WebhookCreate' => $this->serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️WebhookCreate($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️WebhookCreate(mixed $object): mixed
    {
        assert($object instanceof WebhookCreate);
        $result = [];
        ...
        $pusherType                                     = $object->pusherType;
        after_pusherType:        $result['pusher_type'] = $pusherType;

        $ref                             = $object->ref;
        after_ref:        $result['ref'] = $ref;

        $refType                                  = $object->refType;
        after_refType:        $result['ref_type'] = $refType;
