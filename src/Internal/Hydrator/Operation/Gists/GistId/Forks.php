...
use ApiClients\Client\GitHubAE\Schema\BaseGist\Files;
...
class Forks implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️BaseGist(array $payload): BaseGist
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(BaseGist::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new BaseGist(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\BaseGist', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️BaseGist⚡️Files(array $payload): Files
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof BaseGist);
        $result = [];
