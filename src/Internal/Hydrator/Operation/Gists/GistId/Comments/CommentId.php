...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Gists\GistId\Comments;
...
class CommentId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️GistComment(array $payload): GistComment
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(GistComment::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new GistComment(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\GistComment', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof GistComment);
        $result = [];
