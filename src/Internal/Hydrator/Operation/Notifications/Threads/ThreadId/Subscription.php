...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Notifications\Threads\ThreadId;
...
class Subscription implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️ThreadSubscription(array $payload): ThreadSubscription
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ThreadSubscription::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ThreadSubscription(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\ThreadSubscription', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ThreadSubscription);
        $result = [];
