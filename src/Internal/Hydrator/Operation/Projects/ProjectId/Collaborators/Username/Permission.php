...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Projects\ProjectId\Collaborators\Username;
...
class Permission implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️ProjectCollaboratorPermission(array $payload): ProjectCollaboratorPermission
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ProjectCollaboratorPermission::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ProjectCollaboratorPermission(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\ProjectCollaboratorPermission', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ProjectCollaboratorPermission);
        $result = [];
