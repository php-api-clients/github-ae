...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Projects\ProjectId;
...
class Columns implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️ProjectColumn(array $payload): ProjectColumn
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ProjectColumn::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ProjectColumn(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\ProjectColumn', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ProjectColumn);
        $result = [];
