...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\CheckSuites;
...
use ApiClients\Client\GitHubAE\Schema\MinimalRepository\Permissions;
...
class Preferences implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️CheckSuitePreference(array $payload): CheckSuitePreference
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CheckSuitePreference::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CheckSuitePreference(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\CheckSuitePreference', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️MinimalRepository⚡️Permissions(array $payload): Permissions
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof CheckSuitePreference);
        $result = [];
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️CheckSuitePreference⚡️Preferences(mixed $object): mixed
    {
        ...
        if ($autoTriggerChecksSerializer0 === null) {
            $autoTriggerChecksSerializer0 = new SerializeArrayItems(...[]);
        }
