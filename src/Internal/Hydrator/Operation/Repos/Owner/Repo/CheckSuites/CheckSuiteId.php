...
use ApiClients\Client\GitHubAE\Schema\SimpleCommit;
use ApiClients\Client\GitHubAE\Schema\SimpleCommit\Author;
use ApiClients\Client\GitHubAE\Schema\SimpleCommit\Committer;
...
class CheckSuiteId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️CheckSuite(array $payload): CheckSuite
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SimpleCommit(array $payload): SimpleCommit
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(SimpleCommit::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new SimpleCommit(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\SimpleCommit', $exception, stack: $this->hydrationStack);
        }
    }

    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SimpleCommit⚡️Author(array $payload): Author
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SimpleCommit⚡️Committer(array $payload): Committer
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof SimpleCommit);
        $result = [];
