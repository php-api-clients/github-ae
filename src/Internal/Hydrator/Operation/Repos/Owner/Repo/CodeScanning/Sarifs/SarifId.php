...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\CodeScanning\Sarifs;
...
class SarifId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️CodeScanningSarifsStatus(array $payload): CodeScanningSarifsStatus
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodeScanningSarifsStatus::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodeScanningSarifsStatus(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\CodeScanningSarifsStatus', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CodeScanningSarifsStatus);
        $result = [];
