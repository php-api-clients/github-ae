...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\Issues\Comments;
...
class CommentId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️IssueComment(array $payload): IssueComment
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(IssueComment::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new IssueComment(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\IssueComment', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof IssueComment);
        $result = [];
