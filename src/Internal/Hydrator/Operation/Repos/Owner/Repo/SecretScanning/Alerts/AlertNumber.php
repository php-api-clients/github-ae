...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\SecretScanning\Alerts;
...
class AlertNumber implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SecretScanningAlert(array $payload): SecretScanningAlert
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(SecretScanningAlert::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new SecretScanningAlert(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\SecretScanningAlert', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof SecretScanningAlert);
        $result = [];
