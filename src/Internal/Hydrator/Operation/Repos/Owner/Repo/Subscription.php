...
class Subscription implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️RepositorySubscription(array $payload): RepositorySubscription
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(RepositorySubscription::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new RepositorySubscription(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\RepositorySubscription', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof RepositorySubscription);
        $result = [];
