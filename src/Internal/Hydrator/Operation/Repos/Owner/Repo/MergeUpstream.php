...
class MergeUpstream implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️MergedUpstream(array $payload): MergedUpstream
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(MergedUpstream::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new MergedUpstream(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\MergedUpstream', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof MergedUpstream);
        $result = [];
