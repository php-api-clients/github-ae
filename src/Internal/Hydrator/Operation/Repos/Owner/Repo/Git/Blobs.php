...
class Blobs implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️ShortBlob(array $payload): ShortBlob
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ShortBlob::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ShortBlob(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\ShortBlob', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ShortBlob);
        $result = [];
