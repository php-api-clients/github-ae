...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\Pages\Builds;

use ApiClients\Client\GitHubAE\Schema\PageBuild;
use ApiClients\Client\GitHubAE\Schema\PageBuild\Error;
...
class BuildId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PageBuild(array $payload): PageBuild
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(PageBuild::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new PageBuild(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\PageBuild', $exception, stack: $this->hydrationStack);
        }
    }

    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PageBuild⚡️Error(array $payload): Error
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof PageBuild);
        $result = [];
