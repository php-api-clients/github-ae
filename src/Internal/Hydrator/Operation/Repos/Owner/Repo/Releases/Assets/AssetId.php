...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\Releases\Assets;
...
class AssetId implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️ReleaseAsset(array $payload): ReleaseAsset
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(ReleaseAsset::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new ReleaseAsset(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\ReleaseAsset', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof ReleaseAsset);
        $result = [];
