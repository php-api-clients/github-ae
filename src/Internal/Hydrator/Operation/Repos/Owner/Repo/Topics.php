...
class Topics implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️Topic(array $payload): Topic
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(Topic::class, $missingFields, stack: $this->hydrationStack);
        }
    ...
    {
        assert($object instanceof Topic);
        $result = [];
        ...
        if ($namesSerializer0 === null) {
            $namesSerializer0 = new SerializeArrayItems(...[]);
        }
