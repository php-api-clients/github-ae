...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Repos\Owner\Repo\Codeowners;
...
class Errors implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️CodeownersErrors(array $payload): CodeownersErrors
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(CodeownersErrors::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new CodeownersErrors(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\CodeownersErrors', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof CodeownersErrors);
        $result = [];
