...
use ApiClients\Client\GitHubAE\Schema\PrivateUser\Plan;
...
class User implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PrivateUser(array $payload): PrivateUser
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\PrivateUser', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(PrivateUser::class, $missingFields, stack: $this->hydrationStack);
        }
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PrivateUser⚡️Plan(array $payload): Plan
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️PrivateUser(mixed $object): mixed
    {
        assert($object instanceof PrivateUser);
        $result = [];
