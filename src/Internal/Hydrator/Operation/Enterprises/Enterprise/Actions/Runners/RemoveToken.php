...
namespace ApiClients\Client\GitHubAE\Internal\Hydrator\Operation\Enterprises\Enterprise\Actions\Runners;

use ApiClients\Client\GitHubAE\Schema\AuthenticationToken;
use ApiClients\Client\GitHubAE\Schema\AuthenticationToken\Permissions;
...
class RemoveToken implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️AuthenticationToken(array $payload): AuthenticationToken
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(AuthenticationToken::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new AuthenticationToken(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\AuthenticationToken', $exception, stack: $this->hydrationStack);
        }
    }

    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️AuthenticationToken⚡️Permissions(array $payload): Permissions
    {
        $properties    = [];
        $missingFields = [];
        try {
    ...
    {
        assert($object instanceof AuthenticationToken);
        $result = [];
