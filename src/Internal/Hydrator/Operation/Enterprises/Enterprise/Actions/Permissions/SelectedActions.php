...
class SelectedActions implements ObjectMapper
{
    ...
    public function hydrateObject(string $className, array $payload): object
    {
        return match ($className) {
            'ApiClients\Client\GitHubAE\Schema\SelectedActions' => $this->hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SelectedActions($payload),
            default => throw UnableToHydrateObject::noHydrationDefined($className, $this->hydrationStack),
        };
    }

    private function hydrateApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SelectedActions(array $payload): \ApiClients\Client\GitHubAE\Schema\SelectedActions
    {
        $properties    = [];
        $missingFields = [];
        try {
            ...
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\SelectedActions', $exception, stack: $this->hydrationStack);
        }

        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(\ApiClients\Client\GitHubAE\Schema\SelectedActions::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new \ApiClients\Client\GitHubAE\Schema\SelectedActions(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\GitHubAE\Schema\SelectedActions', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    public function serializeObjectOfType(object $object, string $className): mixed
    {
        try {
            return match ($className) {
                ...
                'ApiClients\Client\GitHubAE\Schema\SelectedActions' => $this->serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SelectedActions($object),
    ...
    private function serializeObjectApiClients⚡️Client⚡️GitHubAE⚡️Schema⚡️SelectedActions(mixed $object): mixed
    {
        assert($object instanceof \ApiClients\Client\GitHubAE\Schema\SelectedActions);
        $result = [];
