...
final class ListStarred
{
    ...
    private readonly Hydrator\Operation\Gists\Starred $hydrator;
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Gists\Starred $hydrator, string $since, int $perPage = 30, int $page = 1)
    {
        $this->since = $since;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{since}', '{per_page}', '{page}'), array($this->since, $this->perPage, $this->page), self::PATH . '?since={since}&perPage={per_page}&page={page}'));
    }
    /**
