...
final class ListForks
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Forks $hydrator, string $owner, string $repo, string $sort = 'newest', int $perPage = 30, int $page = 1)
    {
        $this->owner = $owner;
        $this->repo = $repo;
        $this->sort = $sort;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{owner}', '{repo}', '{sort}', '{per_page}', '{page}'), array($this->owner, $this->repo, $this->sort, $this->perPage, $this->page), self::PATH . '?sort={sort}&per_page={per_page}&page={page}'));
    }
    /**
