...
final class GetBranch
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Branches\CbBranchRcb $hydrator, string $owner, string $repo, string $branch)
    {
        $this->owner = $owner;
        $this->repo = $repo;
        $this->branch = $branch;
     ...
     */
    public function createResponse(\Psr\Http\Message\ResponseInterface $response) : Schema\BranchWithProtection|Schema\BasicError
    {
        ...
        switch ($contentType) {
            case 'application/json':
                ...
                switch ($code) {
                    ...
                    **/
                    case 200:
                        $this->responseSchemaValidator->validate($body, \cebe\openapi\Reader::readFromJson(Schema\BranchWithProtection::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
