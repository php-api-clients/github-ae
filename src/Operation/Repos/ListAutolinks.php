...
final class ListAutolinks
{
    ...
    public function __construct(SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Autolinks $hydrator, string $owner, string $repo, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
        $this->page                    = $page;
                    ...
                    **/
                    case 200:
                        foreach ($body as $bodyItem) {
                            $this->responseSchemaValidator->validate($bodyItem, Reader::readFromJson(Schema\Autolink::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
                        }
