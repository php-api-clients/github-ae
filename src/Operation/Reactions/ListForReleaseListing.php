...
final class ListForReleaseListing
{
    ...
    public function __construct(private readonly SchemaValidator $responseSchemaValidator, private readonly Hydrator\Operation\Repos\Owner\Repo\Releases\ReleaseId\Reactions $hydrator, string $owner, string $repo, int $releaseId, string $content, int $perPage = 30, int $page = 1)
    {
        $this->owner     = $owner;
        $this->repo      = $repo;
    ...
    {
        return new Request(self::METHOD, str_replace(['{owner}', '{repo}', '{release_id}', '{content}', '{per_page}', '{page}'], [$this->owner, $this->repo, $this->releaseId, $this->content, $this->perPage, $this->page], self::PATH . '?content={content}&per_page={per_page}&page={page}'));
    }
