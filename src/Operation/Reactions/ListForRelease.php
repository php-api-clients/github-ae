...
final class ListForRelease
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Releases\CbReleaseIdRcb\Reactions $hydrator, string $owner, string $repo, int $releaseId, string $content, int $perPage = 30, int $page = 1)
    {
        $this->owner = $owner;
        $this->repo = $repo;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{owner}', '{repo}', '{release_id}', '{content}', '{per_page}', '{page}'), array($this->owner, $this->repo, $this->releaseId, $this->content, $this->perPage, $this->page), self::PATH . '?content={content}&per_page={per_page}&page={page}'));
    }
    /**
