...
final class ListForIssue
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Issues\CbIssueNumberRcb\Reactions $hydrator, string $owner, string $repo, int $issueNumber, string $content, int $perPage = 30, int $page = 1)
    {
        $this->owner = $owner;
        $this->repo = $repo;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{owner}', '{repo}', '{issue_number}', '{content}', '{per_page}', '{page}'), array($this->owner, $this->repo, $this->issueNumber, $this->content, $this->perPage, $this->page), self::PATH . '?content={content}&per_page={per_page}&page={page}'));
    }
    /**
