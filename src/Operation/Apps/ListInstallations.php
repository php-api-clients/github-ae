...
final class ListInstallations
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\App\Installations $hydrator, string $since, string $outdated, int $perPage = 30, int $page = 1)
    {
        $this->since = $since;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{since}', '{outdated}', '{per_page}', '{page}'), array($this->since, $this->outdated, $this->perPage, $this->page), self::PATH . '?since={since}&outdated={outdated}&per_page={per_page}&page={page}'));
    }
    /**
