...
final class CommitsListing
{
    public const OPERATION_ID = 'search/commits';
    public const OPERATION_MATCH = 'LIST /search/commits';
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Search\Commits $hydrator, string $q, string $sort, string $order = 'desc', int $per_page = 30, int $page = 1)
    {
        $this->q = $q;
        $this->sort = $sort;
        $this->order = $order;
     ...
     */
    function createResponse(\Psr\Http\Message\ResponseInterface $response) : Schema\Operation\Search\Commits\Response\Applicationjson\H200
    {
