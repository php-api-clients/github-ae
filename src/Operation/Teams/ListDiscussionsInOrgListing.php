...
final class ListDiscussionsInOrgListing
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Orgs\CbOrgRcb\Teams\CbTeamSlugRcb\Discussions $hydrator, string $org, string $team_slug, string $pinned, string $direction = 'desc', int $per_page = 30, int $page = 1)
    {
        $this->org = $org;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{org}', '{team_slug}', '{pinned}', '{direction}', '{per_page}', '{page}'), array($this->org, $this->team_slug, $this->pinned, $this->direction, $this->per_page, $this->page), self::PATH . '?pinned={pinned}&direction={direction}&per_page={per_page}&page={page}'));
    }
    /**
