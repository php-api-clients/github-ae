...
final class ListSelfHostedRunnersForRepoListing
{
    public const OPERATION_ID = 'actions/list-self-hosted-runners-for-repo';
    public const OPERATION_MATCH = 'LIST /repos/{owner}/{repo}/actions/runners';
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Actions\Runners $hydrator, string $owner, string $repo, int $per_page = 30, int $page = 1)
    {
        $this->owner = $owner;
        $this->repo = $repo;
     ...
     */
    function createResponse(\Psr\Http\Message\ResponseInterface $response) : Schema\Operation\Actions\ListSelfHostedRunnersForOrg\Response\Applicationjson\H200
    {
