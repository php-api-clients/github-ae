...
final class ListLabelsForMilestone
{
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Milestones\CbMilestoneNumberRcb\Labels $hydrator, string $owner, string $repo, int $milestoneNumber, int $perPage = 30, int $page = 1)
    {
        $this->owner = $owner;
        $this->repo = $repo;
    ...
    {
        return new \RingCentral\Psr7\Request(self::METHOD, \str_replace(array('{owner}', '{repo}', '{milestone_number}', '{per_page}', '{page}'), array($this->owner, $this->repo, $this->milestoneNumber, $this->perPage, $this->page), self::PATH . '?per_page={per_page}&page={page}'));
    }
    /**
