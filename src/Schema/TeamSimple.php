...
final readonly class TeamSimple
{
    ...
    public const SCHEMA_EXAMPLE_DATA = '{"id":1,"nodeId":"MDQ6VGVhbTE=","url":"https:\\/\\/api.github.com\\/organizations\\/1\\/team\\/1","membersUrl":"https:\\/\\/api.github.com\\/organizations\\/1\\/team\\/1\\/members{\\/member}","name":"Justice League","description":"A great team.","permission":"admin","privacy":"closed","htmlUrl":"https:\\/\\/github.com\\/orgs\\/rails\\/teams\\/core","repositoriesUrl":"https:\\/\\/api.github.com\\/organizations\\/1\\/team\\/1\\/repos","slug":"justice-league","ldapDn":"uid=example,ou=users,dc=github,dc=com"}';
    /**
     ...
     */
    public function __construct(public int $id, #[\EventSauce\ObjectHydrator\MapFrom('node_id')] public string $nodeId, public string $url, #[\EventSauce\ObjectHydrator\MapFrom('members_url')] public string $membersUrl, public string $name, public ?string $description, public string $permission, public ?string $privacy, #[\EventSauce\ObjectHydrator\MapFrom('html_url')] public string $htmlUrl, #[\EventSauce\ObjectHydrator\MapFrom('repositories_url')] public string $repositoriesUrl, public string $slug, #[\EventSauce\ObjectHydrator\MapFrom('ldap_dn')] public ?string $ldapDn)
    {
    }
}
